    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-5 pb-20 p-4 sm:p-6" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-xs relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!(String(statusStr || "").toLowerCase() === "completed" || String(statusStr || "") === "5") && (
            ...
            {activeProduct && (
              <div className="bg-slate-50/80 border border-slate-200 rounded-xl p-4 flex flex-col md:flex-row gap-5 items-center">
                ...
                <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
                  ...
                  <div>
                    <span className="text-[10px] uppercase font-bold text-slate-400 font-mono">Fabric Spec & Variant</span>
                    <div className="flex items-center gap-2 mt-1">
                      <img
                        src={getFabricImage(activeProduct.fabricId || activeProduct.variant || activeProduct.productName)}
                        alt={activeProduct.variant || "Fabric"}
                        className="w-9 h-9 object-cover rounded-lg border border-slate-200 shadow-2xs flex-shrink-0"
                        onError={(e) => { (e.target as HTMLImageElement).src = "/images/fabrics/FAB-001.jpg"; }}
                      />
                      <div>
                        <p className="text-xs font-bold text-slate-800 leading-tight">
                          {activeProduct.variant || "Standard Fabric"}
                        </p>
                        <span className="text-[10px] text-slate-500 font-mono">
                          {activeProduct.category || "Garment Apparel"}
                        </span>
                      </div>
                    </div>
                  </div>
