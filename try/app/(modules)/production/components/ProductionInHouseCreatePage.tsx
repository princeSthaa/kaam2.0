...
export function ProductionInHouseCreatePage() {
  ...
  const buildPayload = (status: "Draft" | "Active" | "Material Check") => {
    ...
    return {
      ...
      planName: planName || `In-House Buffer Plan ${planId}`,
      demandType: "In-house Stock",
      sourceId: targetWarehouseId || "Central Store",
      sourceName: mockWarehouses.find(w => w.id === targetWarehouseId)?.name || "Central Store",
      priority: "Normal",
      status: finalStatus,
      plannedStartDate: first.plannedStartDate || planDate,
      plannedCompletionDate: first.plannedCompletionDate || planDate,
      quantity: totalUnits,
      supervisor: "Unassigned",
      productionLine: "Sewing Line 1",
      materialWarehouse: "Central Store",
      productionNotes: inHouseReason || "In-house stock replenishment run.",
      planDate: planDate || new Date().toISOString().split("T")[0],
      outputDestination: mockWarehouses.find(w => w.id === targetWarehouseId)?.name || "Central Store",
      requiredDate: first.requiredDate || planDate,
      progress: 0,
      blocked: false,
      createdAt: new Date().toISOString(),
      productionPlanProducts: selectedProducts.flatMap((prod, idx) => prod.variants.map((v, vi) => {
        ...
        return {
          ...
          plannedStartDate: prod.plannedStartDate || planDate,
          plannedCompletionDate: prod.plannedCompletionDate || planDate,
          requiredDate: prod.requiredDate || planDate,
          status: finalStatus,
          productionNotes: prod.productionNotes || inHouseReason || "In-house buffer run",
          createdAt: new Date().toISOString(),
          productionPlanProductSizes: Object.entries(v.sizes).map(([size, qty], sIdx) => ({
      ...
      productionPlanStages: (first.stages || []).map((s: Stage, sIdx: number) => ({
        id: `STG-${Date.now()}-${sIdx}-${Math.random().toString(36).substring(2, 7)}`,
        stageId: `STG-${String(sIdx + 1).padStart(2, "0")}`,
        stageName: s.name,
        workCenter: s.workCenter,
        plannedStartDate: s.date || planDate,
        plannedEndDate: s.date || planDate,
        status: "Not Started",
        completedQty: 0,
        rejectedQty: 0,
