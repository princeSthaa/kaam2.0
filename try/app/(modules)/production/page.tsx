...
export default function ProductionOverviewDashboardPage() {
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      urgentCount,
      yieldRate,
      onTimeDelivery: 0 // Replace mock value with 0 until implemented
    };
  }, [plans]);
  ...
  const bottleneckData = useMemo(() => {
    ...
    // Do not return mock data, just return the calculated stageLoads
  ...
  // Chart 4: Daily Output vs Target (Removed mock data)
  const outputData = useMemo(() => {
     // TODO: Calculate actual output data from backend plans
     return [];
  }, [plans]);
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 min-h-[calc(100vh-64px)] p-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex gap-3">
          <Link href="/production/plans" className="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 transition-colors shadow-sm font-medium">
            View All Plans
          </Link>
          <Link href="/production/demands" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200 font-medium flex items-center gap-2">
