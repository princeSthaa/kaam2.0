...
export function ProductionInHouseCreatePage() {
  ...
  const buildPayload = (status: "Draft" | "Material Check") => {
    ...
    return {
      id: planId, planId, planNo: planId, planDate, demandType: "In-house Stock", planName,
      sourceId: targetWarehouseId, sourceName: mockWarehouses.find(w => w.id === targetWarehouseId)?.name || "",
      ...
      products: selectedProducts.flatMap(prod => prod.variants.map((v, vi) => ({
        lineId: `${planId}-${prod.productId}-V${vi + 1}`, productId: prod.productId, productCode: prod.productCode, productName: prod.productName,
        category: "Garment", variant: v.fabricName, quantity: Object.values(v.sizes).reduce((a, b) => a + b, 0),
        requiredDate: prod.requiredDate, status, sizes: Object.entries(v.sizes).map(([size, qty]) => ({ size, quantity: Number(qty) })), productionNotes: prod.productionNotes,
      }))),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (typeof window !== "undefined") {
      const list = JSON.parse(localStorage.getItem("productionPlans") || "[]");
      list.unshift(plan);
      localStorage.setItem("productionPlans", JSON.stringify(list));
    }
    setTimeout(() => { setIsSubmitting(false); alert("In-house production plan created!"); router.push("/Production/Plan/PlansDetails"); }, 800);
  };
    ...
    <div style={S.page}>
      ...
      <div style={S.hero}>
        ...
        <div style={S.heroActions}>
          <Link href="/Production/Create" style={S.btnOutlineWhite}>
            <span className="material-symbols-outlined" style={{ fontSize: 16 }}>swap_horiz</span>
            Change Type
          </Link>
          <Link href="/Production/Plan/PlansDetails" style={S.btnSolidWhite}>
      ...
      <div style={{ ...S.stepRow, borderRadius: 14, overflow: "hidden", border: "1.5px solid #e5e7eb", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        {steps.map((step, i) => (
          <div key={i} style={{ ...S.stepItem(i <= filledStep), borderLeft: i === 0 ? "none" : "1px solid #e5e7eb", borderRight: "none" }}>
        ...
        {selectedProducts.length > 0 && (
          <div style={S.summaryBar}>
            ...
            <div style={{ display: "flex", gap: 10, flexShrink: 0 }}>
              <button type="button" onClick={() => router.push("/Production/Create")} style={S.btnSecondary}>Cancel</button>
