...
export function ProductionPlanEditPage() {
  ...
  const materialsRequirements = useMemo(() => {
    ...
    selectedProducts.forEach(prod => {
      const totalQty = prod.variants.reduce((s: number, v: any) => s + Object.values(v.sizes).reduce((a: number, b: any) => a + Number(b), 0), 0);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedPlan = {
      ...
      products: selectedProducts.flatMap(prod => prod.variants.map((v: any, vi: number) => ({
        ...
        quantity: Object.values(v.sizes).reduce((a: number, b: any) => a + Number(b), 0),
