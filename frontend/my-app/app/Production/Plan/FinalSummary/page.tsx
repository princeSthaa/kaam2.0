...
export default function FinalSummaryPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5083/api/production-plans")
      .then(res => res.json())
      .then(plans => {
        const found = plans.find((p: any) => p.planId === planId);
        if (found) {
          setPlan(found);
  ...
  const handleConfirmCreate = () => {
    if (typeof window !== "undefined" && planGuid && plan) {
      const updatedPlan = { ...plan, status: "Active", batchId };
  ...
  const kpis = [
    { label: "Total Output",  value: `${totalQty.toLocaleString()} units`,            icon: "inventory_2", bg: "bg-blue-50",    ring: "ring-blue-100",    text: "text-blue-700"    },
    { label: "Est. Cost",     value: `Rs. ${estimatedCost.toLocaleString()}`,          icon: "payments",    bg: "bg-emerald-50", ring: "ring-emerald-100", text: "text-emerald-700" },
    { label: "SKUs",          value: String(productCount),                             icon: "category",    bg: "bg-violet-50",  ring: "ring-violet-100",  text: "text-violet-700"  },
    { label: "Lead Time",     value: daysToComplete ? `${daysToComplete} days` : "TBD", icon: "schedule",  bg: "bg-amber-50",   ring: "ring-amber-100",   text: "text-amber-700"   },
  ];
  ...
  return (
    <>
      ...
      >
        {/* ── Scrollable content ── */}
        <div className="p-4 sm:p-6 pb-28 lg:pb-10 w-full max-w-6xl mx-auto text-slate-800">
          ...
          <div className="no-print flex flex-wrap items-center gap-2 sm:gap-3 mb-6">
            {([
              ...
              <div key={b.label} className="relative group">
                <button onClick={b.fn}
                  className="flex items-center gap-1.5 px-3 sm:px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-600 text-xs sm:text-sm font-medium hover:bg-slate-50 active:scale-[.97] transition-all shadow-sm">
            ...
            <div className="ml-auto relative group hidden sm:block">
              <button onClick={handleConfirmCreate}
                className="flex items-center gap-2 px-5 py-2.5 bg-kaam-primary text-white rounded-lg text-sm font-bold hover:opacity-90 active:scale-[.97] transition-all shadow-md">
          ...
          {/* ════════ DOCUMENT HEADER ════════ */}
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 mb-5 flex flex-col sm:flex-row gap-5 sm:items-start sm:justify-between">
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Production Plan Summary</p>
              <h1 className="text-lg sm:text-2xl font-bold text-slate-900 leading-snug truncate">{plan.planName}</h1>
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <span className="text-[11px] text-slate-500">{plan.planDate}</span>
            ...
            {/* Batch badge */}
            <div className="flex-shrink-0 bg-slate-800 rounded-xl px-5 py-4 flex flex-col gap-1.5 self-start">
              <p className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Batch ID</p>
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleCopy} title="Copy Batch ID"
                  className={`p-1 rounded transition-colors ${copied ? "text-emerald-400" : "text-slate-400 hover:text-white"}`}>
                  <span className="material-symbols-outlined text-[18px]">{copied ? "check" : "content_copy"}</span>
                </button>
              </div>
              <p className="text-[10px] text-slate-500">Plan: {planId}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-5">
            {kpis.map(k => (
              <div key={k.label} className={`kpi-card bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col gap-3`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
                <h2 className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-4 flex items-center gap-2">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="px-5 sm:px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-2">
                ...
                <div className="sm:hidden divide-y divide-slate-100">
                  {plan.products?.map((prod: any, i: number) => (
                    <div key={i} className="p-4 flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-900 text-sm leading-snug truncate">{prod.productName}</p>
                        <p className="text-[11px] text-slate-400 mt-0.5">{prod.productCode} · {prod.variant}</p>
                        <p className="text-[11px] text-slate-500 font-mono mt-0.5">{prod.orderNo}</p>
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {prod.sizes?.map((sz: any) => (
                            <span key={sz.size} className="bg-slate-100 border border-slate-200 text-[10px] px-1.5 py-0.5 rounded font-bold text-slate-600">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="text-lg font-bold text-slate-900">{prod.quantity}</p>
                ...
                <div className="hidden sm:block overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[580px]">
                    <thead className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase tracking-wider text-slate-400 font-bold">
                      <tr>
                        ...
                        <tr key={i} className="hover:bg-slate-50/70 transition-colors">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-semibold text-slate-900 leading-tight">{prod.productName}</p>
                                <p className="text-[11px] text-slate-400 mt-0.5">{prod.productCode} · {prod.category}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-5 py-4 text-slate-600 align-middle">{prod.variant || "—"}</td>
                          <td className="px-5 py-4 text-slate-500 text-xs align-middle font-mono">{prod.orderNo}</td>
                          <td className="px-5 py-4 text-right align-middle">
                            <span className="text-xl font-bold text-slate-900">{prod.quantity}</span>
                          </td>
                          <td className="px-5 py-4 align-middle">
                            <div className="flex flex-wrap gap-1.5">
                              {prod.sizes?.map((sz: any) => (
                                <span key={sz.size}
                                  className="bg-slate-100 border border-slate-200 text-[10px] px-1.5 py-0.5 rounded font-bold text-slate-600">
                                  {sz.size}<span className="text-kaam-primary">:{sz.quantity}</span>
                                </span>
                              ))}
                            </div>
                ...
                <div className="px-5 sm:px-6 py-3 bg-slate-50 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2 text-xs sm:text-sm">
                  <span className="text-slate-400 font-bold uppercase text-[10px] tracking-wider">
                    Estimated Production Cost <span className="normal-case font-normal">(Rs. {costPerUnit.toLocaleString()}/unit)</span>
                  </span>
                  <span className="font-bold text-emerald-700 text-sm sm:text-base">Rs. {estimatedCost.toLocaleString()}</span>
                </div>
              </div>

              {/* Stage pipeline */}
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
                <h2 className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-5 flex items-center gap-2">
                  ...
                  {plan.stages?.map((st: any, i: number) => (
                    <div key={st.stageId} className="flex gap-4 relative">
                      {i < plan.stages.length - 1 && (
                        <div className="absolute left-[15px] top-[34px] bottom-0 w-[2px] bg-slate-100 z-0"></div>
                      )}
                      <div className="relative z-10 w-8 h-8 rounded-full bg-kaam-primary text-white flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 shadow-sm">
                        {i + 1}
                      </div>
                      <div className={`flex-1 pb-5 ${i < plan.stages.length - 1 ? "border-b border-slate-100" : ""}`}>
                        <div className="flex flex-wrap items-center gap-2">
                          <p className="font-semibold text-slate-900">{st.stageName}</p>
                          <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-bold">{st.stageId}</span>
                        </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              {/* Production Setup */}
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6">
                <h2 className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1 flex items-center gap-2">
                  <span className="material-symbols-outlined text-[15px]">settings</span>
                  Production Setup
                </h2>
                <p className="text-[11px] text-slate-400 mb-4">The team, line, and warehouse assigned to run this plan.</p>
                  ...
                  {[
                    ...
                    <div key={f.label} className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
                        <span className="material-symbols-outlined text-[15px] text-slate-500">{f.icon}</span>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">{f.label}</p>
                        <p className="text-sm font-semibold text-slate-800 mt-0.5">{f.val}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex gap-3 items-center">
                    <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                      <span className="material-symbols-outlined text-blue-600 text-[15px]">play_circle</span>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Start</p>
                      <p className="text-sm font-semibold text-slate-800">{plan.plannedStartDate || "TBD"}</p>
                    </div>
                  </div>
                  <div className="w-0.5 h-4 bg-slate-200 ml-4"></div>
                  <div className="flex gap-3 items-center">
                    <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
                      <span className="material-symbols-outlined text-emerald-600 text-[15px]">flag_circle</span>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Completion</p>
                      <p className="text-sm font-semibold text-slate-800">{plan.plannedCompletionDate || "TBD"}</p>
                    </div>
                  </div>
                  {daysToComplete !== null && (
                    <div className="mt-1 bg-slate-50 rounded-xl border border-slate-100 px-4 py-3 flex items-center justify-between">
                      <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Lead Time</p>
                      <p className="text-xl font-bold text-slate-800">{daysToComplete} <span className="text-sm font-medium text-slate-500">days</span></p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="mt-8 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="h-1 w-full bg-gradient-to-r from-emerald-400 via-emerald-500 to-teal-400"></div>
            <div className="p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
              ...
              <div className="flex items-center gap-4 flex-1">
                <div className="w-11 h-11 rounded-full bg-emerald-50 border border-emerald-200 flex items-center justify-center flex-shrink-0">
                  <span className="material-symbols-outlined text-emerald-500 text-[22px]">verified</span>
                </div>
                <div>
                  <p className="font-bold text-slate-900 text-base">Ready to go — plan is fully configured</p>
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-3 text-xs">
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Batch ID</p>
                  <p className="font-mono text-slate-800 font-bold mt-0.5">{batchId}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Plan ID</p>
                  <p className="font-mono text-slate-800 font-bold mt-0.5">{planId}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Est. Cost</p>
                  <p className="font-bold text-emerald-600 mt-0.5">Rs. {estimatedCost.toLocaleString()}</p>
                </div>
              </div>
              ...
              <button
                ...
                className="no-print flex-shrink-0 w-full sm:w-auto flex items-center justify-center gap-2 px-7 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-all active:scale-[.97] shadow-md hover:shadow-lg hover:shadow-emerald-200 text-sm"
              >
